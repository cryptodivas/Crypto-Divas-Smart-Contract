...
error PresaleDeactivated();

contract CryptoDivas is Ownable, ERC721, ERC2981, Treasury {
  ...
  event WhitelistLimitUpdated(uint256 newUPPERLIMIT);
  event PublicSaleActivation(bool _activation);
  event PresaleActivation(bool _activation);
  ...
  ERC721("Crypto Divas Official", "CRYPTODIVAS")
  ...
  function buyPresale(
    ...
    if(!presaleOpen) revert PresaleDeactivated();
    ...
    for(uint256 i = numberOfTokens; i<= MINT_UPPERLIMIT; i++){
    ...
    require((totalSupply + numberOfTokens) <= MAX_SUPPLY, "Total Supply limit reached");
  ...
  */
  ...
  // Owner can mint the remaining tokens
  function mintTokensOwner(uint256 numberOfTokens) external onlyOwner {
  ...
  */
  ...
  function setWhitelistUpperLimit(uint256 newUPPERLIMIT) external onlyOwner{
    ...
    emit WhitelistLimitUpdated(newUPPERLIMIT);
  ...
  function setPresaleActivation(bool _activation) external onlyOwner{
    presaleOpen = _activation;
    emit PresaleActivation(_activation);
    ...
    onlyOwner
  {
    ...
    require(supplyLimit <= MAX_SUPPLY, "Invalid supply limit");
  ...
  function rollStartIndex() external onlyOwner {
    ...
    randomizedStartIndex = (number % MAX_SUPPLY) + 1;
  ...
  */
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, (((tokenId + randomizedStartIndex) % MAX_SUPPLY) + 1).toString(), ".json") ) : dummyURI;
  }
